...
class LogBehaviorController extends Controller
{
    ...
    public function saveListAppForCheck(Request $request)
    {
        $country = $request->get('country');
        $app = $request->get('app');
        $platform = $request->get('platform');
        ...
        $getCheck = DB::connection('mongodb')
            ->table('app_install')
            ->where('country', strtolower($country))
            ->where('app', strtolower($app))
        ...
        if (empty($getCheck)) {
            ...
            $data['status'] = true;
            $data['id_app_install'] = (string) new \MongoDB\BSON\ObjectId($idAppInstall);
        } else {
            $data['status'] = false;
        }
    ...
    public function deleteAppInListForCheck(Request $request)
    {
        $id = $request->get('id');
    ...
    public function searchAppInListForCheck(Request $request)
    {
        ...
        foreach ($query as $record) {
            if ($record['last_install'] == '') {
                $record['last_install'] = 'Chưa có lượt cài mới';
            } else {
                $record['last_install'] = $record['last_install']->toDateTime()->format('Y-m-d H:i:s');
            }
    ...
    public function lockAppInListForCheck(Request $request)
    {
        $id = $request->get('id');
        $dataUpdate = [
