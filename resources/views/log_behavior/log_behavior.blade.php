...
<div class="py-8">
    <div class="">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="relative group">
                                    <input type="date" id="datepicker"
                                        ...
                                        placeholder="{{ __('Chọn ngày') }}" value="{{ $filter['date'] ?? '' }}">
                                    <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">
                                        <svg class="h-5 w-5 text-gray-400 group-hover:text-indigo-500 peer-focus:text-indigo-500"
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                                    ...
                                    <div class="space-y-2 w-40">
                                        <div class="relative group">
                                            <input type="date" id="current-date-picker"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm [&::-webkit-calendar-picker-indicator]:opacity-0 [&::-webkit-calendar-picker-indicator]:absolute [&::-webkit-calendar-picker-indicator]:right-0 [&::-webkit-calendar-picker-indicator]:w-10"
                                                placeholder="{{ __('Chọn ngày') }}" max="{{ date('Y-m-d') }}"
                                                value="{{ $filter['date'] ?? '' }}">
                                            <div
                                ...
                                <div class="bg-gray-50 rounded-lg p-4">
                                    <dl class="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <dd id="current-total-installs"
                                                class="mt-1 text-lg font-semibold text-gray-900">-</dd>
                                        </div>
                                        <div>
                                            <dt class="text-xs text-gray-500">{{ __('Thành công') }}</dt>
                                            <dd id="current-success-installs"
                                                class="mt-1 text-lg font-semibold text-gray-900">-</dd>
                                        </div>
                                        <div>
                                            <dt class="text-xs text-gray-500">{{ __('Sai quốc gia') }}</dt>
                                            <dd id="current-wrong-country"
                                                class="mt-1 text-lg font-semibold text-gray-900">-</dd>
                                        </div>
                                        <div>
                                            <dt class="text-xs text-gray-500">{{ __('Sai mạng') }}</dt>
                                            <dd id="current-wrong-network"
                                                class="mt-1 text-lg font-semibold text-gray-900">-</dd>
                                        </div>
                                    </dl>
                                </div>
                            ...
                            <div class="space-y-4">
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="space-y-2 w-40">
                                        <div class="relative group">
                                            <input type="date" max="{{ date('Y-m-d') }}" id="comparison-date"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm [&::-webkit-calendar-picker-indicator]:opacity-0 [&::-webkit-calendar-picker-indicator]:absolute [&::-webkit-calendar-picker-indicator]:right-0 [&::-webkit-calendar-picker-indicator]:w-10"
                                ...
                                <div class="bg-gray-50 rounded-lg p-4">
                                    <dl class="grid grid-cols-2 gap-4">
                                        <div>
                                            <dt class="text-xs text-gray-500">{{ __('Tổng lượt cài') }}</dt>
                                            <dd id="comparison-total-installs"
                                                class="mt-1 text-lg font-semibold text-gray-900">-</dd>
                                        </div>
                                        <div>
                                            <dt class="text-xs text-gray-500">{{ __('Thành công') }}</dt>
                                            <dd id="comparison-success-installs"
                                                class="mt-1 text-lg font-semibold text-gray-900">-</dd>
                                        </div>
                                        <div>
                                            <dt class="text-xs text-gray-500">{{ __('Sai quốc gia') }}</dt>
                                            <dd id="comparison-wrong-country"
                                                class="mt-1 text-lg font-semibold text-gray-900">-</dd>
                                        </div>
                                        <div>
                                            <dt class="text-xs text-gray-500">{{ __('Sai mạng') }}</dt>
                                            <dd id="comparison-wrong-network"
                                                class="mt-1 text-lg font-semibold text-gray-900">-</dd>
                                        </div>
                                    </dl>
                                </div>
        ...
        <!-- Data Table -->
        <div class="mt-4 custom-data-table">
            @if (count($data) > 0)
            <x-table>
                    ...
                    <x-table.row class="hover:bg-gray-50">
                        ...
                        <x-table.cell class="max-w-md whitespace-normal">
                            ...
                            echo '<div class="space-y-1.5">';
                                ...
                                strpos($value, 'SUCCESS') !== false ||
                                strpos($key, 'SUB_OK') !== false
                                ) {
                                $bgColor = 'bg-blue-50';
                                $textColor = 'text-blue-700';
                                } elseif (
                                strpos($key, 'ERRO') !== false ||
                                strpos($value, 'ERRO') !== false ||
                                strpos($key, 'SAI_') !== false
                                ) {
                                $bgColor = 'bg-red-50';
                                ...
                                echo "<div class='text-xs p-1.5 rounded {$bgColor} break-words'>";
                                    ...
                                    echo "<span class='break-all'>" .
                                        e(trim($parts[0])) .
                                        '</span>';
                                    if (isset($parts[1])) {
                                    echo "<div class='mt-1 font-mono text-xs text-gray-500 break-all'>DEVICE: " .
                                        e(trim($parts[1])) .
                                        '</div>';
                                    }
                                    } elseif (filter_var($value, FILTER_VALIDATE_URL)) {
                                    echo "<span class='break-all text-blue-600 hover:text-blue-800'>" .
                                        e($value) .
                                        '</span>';
                                    } else {
                            ...
                            echo '<div class="text-xs text-gray-600 break-words">' .
                                e($behavior) .
                                '</div>';
                            }
                            } else {
