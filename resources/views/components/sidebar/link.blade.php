...
@if ($collapsible)
    <button type="button" {{ $attributes->merge(['class' => $classes]) }} x-data="{ tooltipVisible: false }"
        ...
        <div x-cloak x-show="!isSidebarOpen && !isSidebarHovered && tooltipVisible"
            ...
            class="left-[3rem] top-auto bg-gray-800 dark:bg-gray-700 text-white text-xs z-[9999] rounded-md px-2 py-1.5 shadow-lg whitespace-nowrap">
            {{ $title }}
            <div
                class="absolute left-0 top-1/2 -translate-x-1 -translate-y-1/2 border-[6px] border-transparent border-r-gray-800 dark:border-r-gray-700">
            </div>
        </div>
    </button>
@else
    <a {{ $attributes->merge(['class' => $classes]) }} x-data="{ tooltipVisible: false }" @mouseenter="tooltipVisible = true"
        ...
        <div x-cloak x-show="!isSidebarOpen && !isSidebarHovered && tooltipVisible"
            ...
            x-transition:leave-start="opacity-100 translate-x-0" x-transition:leave-end="opacity-0 translate-x-2"
            style="position: fixed;"
            class="left-[3rem] top-auto ml-3 bg-gray-800 dark:bg-gray-700 text-white text-xs z-[9999] rounded-md px-2 py-1.5 shadow-lg whitespace-nowrap">
            {{ $title }}
            <div
                class="absolute left-0 top-1/2 -translate-x-1 -translate-y-1/2 border-[6px] border-transparent border-r-gray-800 dark:border-r-gray-700">
            </div>
