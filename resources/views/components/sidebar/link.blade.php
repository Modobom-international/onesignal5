...
@else
<a {{ $attributes->merge(['class' => $classes]) }} style="padding-left: 5px !important;">
    @if ($icon ?? false)
    {{ $icon }}
    @else
    <x-icons.empty-circle class="flex-shrink-0 w-6 h-6" aria-hidden="true" />
    ...
    <span class="text-base font-medium" x-show="isSidebarOpen || isSidebarHovered" style="border: none;">
        {{ $title }}
    </span>
</a>
@endif